...
for n in N_VALUES:
    ...
    sum_c = {"Insertion": 0, "Quick": 0, "Hybrid": 0}
...
fig.suptitle(f'Porównanie algorytmów sortowania (n ∈ [10, 20, ..., 50], k={K_REPEATS})', fontsize=16)

# Średnia liczba porównań (c)
axs[0, 0].plot(N_VALUES, results["Insertion"]["c"], 'r.-', label='Insertion')
...
axs[0, 1].plot(N_VALUES, results["Insertion"]["s"], 'r.-', label='Insertion')
...
# c / n
axs[1, 0].plot(N_VALUES, results["Insertion"]["c_n"], 'r.-', label='Insertion')
...
# s / n
axs[1, 1].plot(N_VALUES, results["Insertion"]["s_n"], 'r.-', label='Insertion')
...
plt.tight_layout()
plt.savefig(f'plots_small_k{K_REPEATS}.png')
#plt.show()